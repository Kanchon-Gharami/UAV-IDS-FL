# clients/client_cyber_physical.py
...
from preprocessing.preprocess_cyber_physical import get_cyber_physical_dataloaders
...
class CyberPhysicalClient(fl.client.NumPyClient):
    def __init__(self, model, train_loader, test_loader, label_encoder, class_names, device=torch.device("cpu")):
        """
        Initializes the Flower client for the Cyber Physical dataset.
    ...
    def fit(self, parameters, config):
        ...
        print(f"Cyber Physical Round {self.current_round}")
    ...
    def save_results(self, y_true, y_pred, accuracy, precision, recall, f1):
        ...
        # Save plot
        result_dir = f"result/client_cyber_physical/round_{self.current_round}"
...
if __name__ == "__main__":
    # Define class names for Cyber Physical dataset
    # These will be automatically determined by the LabelEncoder
    # Ensure that the test set contains all classes present in the training set
    # If not, handle accordingly to prevent unseen labels
    # Example class names (adjust based on your dataset)
    # class_names = ["Benign", "Under Attack"]

    # Prepare data paths
    train_file = "data/cyber_physical/train.csv"
    test_file = "data/cyber_physical/test.csv"
    ...
    # Load data
    train_loader, test_loader, input_dim, num_classes, label_encoder = get_cyber_physical_dataloaders(
